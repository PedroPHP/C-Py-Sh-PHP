...
verificar_subdominios_com_extensao() {
    ...
    while IFS= read -r subdomain; do
        ...
        http_status=$(curl -s -o /dev/null -w "%{http_code}" -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" "$domain/$subdomain.$ext")
...
verificar_subdominios_sem_extensao() {
    echo "Descobrindo subdominios de $domain a partir de $lista sem extensão"
    ...
    while IFS= read -r subdomain; do
        subdomain=$(basename "$subdomain")  # Remove o caminho completo
        http_status=$(curl -s -o /dev/null -w "%{http_code}" -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" "$domain/$subdomain")
        if [ $http_status -eq 200 ]; then
            status_200["$domain/$subdomain"]="Status: $http_status"
            resolver_dns "$subdomain.$domain"
        elif [ $http_status -eq 301 ]; then
...
http_status=$(curl -s -o /dev/null -w "%{http_code}" -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" "$domain")
